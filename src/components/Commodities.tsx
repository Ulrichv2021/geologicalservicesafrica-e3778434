...
export function Commodities() {
  ...
  return (
    <section id="commodities" className="relative py-28 lg:py-36 overflow-hidden">
      {/* Video Background */}
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10" ref={ref}>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {commodities.map((commodity, index) => (
            ...
            >
              {/* Header */}
              <div className={`bg-gradient-to-r ${commodity.gradient} p-6`}>
                <commodity.icon className="w-12 h-12 text-white mb-4" />
                <h3 className="font-display text-2xl text-white mb-1">{commodity.name}</h3>
                <p className="text-white/80 text-sm font-mono">{commodity.formula}</p>
              </div>
              
              {/* Content */}
              <div className="p-6 space-y-6">
                <p className="text-white/70 leading-relaxed">{commodity.description}</p>
                
                <div>
                  <div className="text-xs text-white/50 uppercase tracking-wider mb-3">Key Applications</div>
